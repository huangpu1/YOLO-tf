...
def conv2d(idx, input_volume, kernel, name, alpha, stride=1):
	...
	"""
	print 'Layer  %2d : Type = Conv\tKernel = %d * %d\tStride = %d\tFilters = %4d\tInput channels = %4d' % (idx, kernel[0], kernel[1], stride, int(kernel[3]), int(kernel[2]))
...
def max_pool(idx, input_volume, kernel=2, stride=2, name=None):
	...
	"""	
	print 'Layer  %2d : Type = Pool\tKernel = %d * %d\tStride = %d' % (idx, kernel, kernel, stride)
...
def fully_connected(idx, _input, _output, name, alpha, activation=None):
	...
	"""
	print 'Layer  %2d : Type = Full\tInput dimension = %4d\tOutput dimension = %4d' % (idx, int(_input.get_shape()[1]), _output)
